...
export const FeiraEditForm = ({ feiraId, open, onOpenChange, onSuccess }: FeiraEditFormProps) => {
    ...
    try {
      ...
      if (data) {
        setFormData({
          ...
          tipo_feira: data.tipo_feira || "publica",
          dias_semana: data.dias_semana || [],
          horario_inicio: data.horario_inicio || "",
          horario_fim: data.horario_fim || "",
          tempo_antecedencia_minutos: data.tempo_antecedencia_minutos || 30,
          prazo_pagamento_dias: data.prazo_pagamento_dias || 3,
          formas_pagamento: data.formas_pagamento || ["pix", "debito", "credito"],
          horas_cancelamento_sem_multa: data.horas_cancelamento_sem_multa || 24,
          taxa_cancelamento: data.taxa_cancelamento?.toString() || "",
          valor_participacao: data.valor_participacao?.toString() || "",
          taxa_energia: data.taxa_energia?.toString() || "",
          taxa_limpeza: data.taxa_limpeza?.toString() || "",
          taxa_seguranca: data.taxa_seguranca?.toString() || "",
          regras_evento: data.regras_evento || "",
          politica_cancelamento: data.politica_cancelamento || "",
          avisos: data.avisos || "",
          observacoes: data.observacoes || "",
          recorrente: data.recorrente || false,
          segmento_exclusivo: data.segmento_exclusivo || false,
        });
      }
    } catch (error: any) {
      toast.error("Erro ao carregar dados da feira: " + error.message);
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Editar Feira</DialogTitle>
        </DialogHeader>
